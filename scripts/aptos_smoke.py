"""End-to-end APTOS sanity check (small): 5 clients, 2 rounds, ResNet-18.

Requires APTOS data at ``data_cache/aptos2019/``. Run
``python scripts/download_aptos.py`` first if missing.
"""
import os
import sys

HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.dirname(HERE))

import torch

from config import get_config
from data import build_federated_datasets
from federated import Client, Server
from models import build_model
from utils import set_seed


def run():
    device = (
        "cuda" if torch.cuda.is_available()
        else "mps" if torch.backends.mps.is_available()
        else "cpu"
    )
    cfg = get_config(
        dataset="aptos",
        model="resnet18",
        pretrained=True,
        num_classes=5,
        image_size=224,
        num_clients=5,
        alpha=0.5,
        num_rounds=2,
        local_epochs=1,
        candidate_size_d=4,
        active_size_m=2,
        batch_size=16,
        num_workers=2,
        device=device,
        exp_name="aptos_smoke",
    )
    set_seed(cfg.seed)
    print(f"[smoke] device={cfg.device}")
    subsets, test_set, labels = build_federated_datasets(cfg)
    print(f"[smoke] train clients: sizes={[len(s) for s in subsets]} | test={len(test_set)}")
    clients = [Client(i, subsets[i], labels[i], cfg) for i in range(cfg.num_clients)]
    server = Server(cfg, clients, test_set, lambda: build_model(cfg))
    for r in range(cfg.num_rounds):
        info = server.run_round(r)
        print(
            f"round {r}: active={info['active']} "
            f"weights={[round(w, 3) for w in info['weights']]}"
        )
    m = server.evaluate()
    print(
        f"eval acc={m['acc']:.3f} loss={m['loss']:.3f} "
        f"qwk={m.get('qwk', float('nan')):.3f}"
    )
    assert 0.0 <= m["acc"] <= 1.0
    print("aptos smoke OK")


if __name__ == "__main__":
    run()
